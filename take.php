...
<?php
  ...
  if(!in_array($class_id,$classes)) die( "No such record." ); // Assuming that we have validated and thrown errors if any , we proceed 
                                                              // By finding the particular object we are talking about 
 
  // Connecting to the database 
  $classNode = new Node;
  $node = $classNode->retrieveObjecti($class_id,$teacher_id) or die("No such record");  // Intimating the teacher about Number of Classes , and student list
 ...
 <body>
  ...
  <div class="container"> 
    <?php 
      ...
      echo '<h3>Class: '.$node->getCode().'   Section:  '.$node->getSection().'   Year: '.$node->getYear().'</h3>';
